...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //the 'data' property. The value is 'songData'. That means ng-grid is looking for songData on $scope and is 
  //putting whatever songData is into the grid.
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function() {
      ...
      itunesService.getSongs($scope.artist).then(function(songs){
        ...
        $scope.songData = songs.map(function(itunesFormatResult){
          return {
            AlbumArt: itunesFormatResult.artworkUrl100,
            Artist: itunesFormatResult.artistName,
            Collection: itunesFormatResult.collectionName,
            CollectionPrice: itunesFormatResult.collectionPrice,
            Play: itunesFormatResult.previewUrl,
            Type: itunesFormatResult.kind
          }
        })
      })
    }
